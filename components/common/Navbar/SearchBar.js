...
export default function SearchBar({ liveSearch }) {
    ...
    function onKeyUp(e) {
        router.push('http://localhost:3000/?title=' + input);
    }
            ...
            <div className="ml-2 flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
                <div className="flex items-center justify-center w-full">
                    <div className="input-group relative flex items-center w-full">
                        <input type="search" value={input} onInput={e => setInput(e.target.value) } onKeyUp={onKeyUp} className="form-control relative flex items-center justify-center min-w-0 block w-full px-3 py-1.5 text-slate-200 font-normal bg-gradient-to-l from-slate-900 via-slate-800 to-slate-900 bg-clip-padding rounded-lg border border-solid border-[#0A2730] transition ease-in-out mr-1 m-0 focus:text-slate-200 focus:bg-[#EFF1F3] hover:border-slate-500 focus:outline-none" aria-label="Search" aria-describedby="button-addon2"></input>
                    </div>
                </div>
            </div>
        )
