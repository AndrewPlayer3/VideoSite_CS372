    ...
    if (req.method === 'GET') {  // Retrieve Video Information | TODO: Restrict these requests to any logged-in user.
        try {
            ...
            if (query_results) {
                ...
                return res.status(404).send({ success: false, video: {} });
            }
        } catch (error) {
            return res.status(500).send({ success: false, video: {}, error: error.message})
        }
        ...
        try {    
            ...
            res.status(500).send({ removed: false, id: vid, message: error.message });
        }
    } else if (req.method == 'PUT') {
        try {
            const view_video = await Video.findByIdAndUpdate(vid, { $inc: { 'analytics.views': 1 } });
